...
const FormUi = ({ jsonForm, selectedTheme, onFieldUpdate, deleteField, selectedStyle, editable=true, formId=0, enableSignIn=false }) => {
  ...
  // Accept either a single form object or an array of forms (pick first)
  ...
  {(form?.fields ?? []).map((field, index) => (
        <div key={index} className="flex items-center gap-2">
          {field.fieldType == "select" ? (
            ...
            <div className="my-3 w-full">
              <label className="text-xs text-gray-500">{field.label}</label>
              <RadioGroup required={field?.required} onValueChange={(v) => handleSelectChange(field.fieldName, v)}>
              ...
              {field?.options ? (
                field?.options?.map((item, idx) => (
                  <div key={idx} className="flex gap-2 items-center">
                    <Checkbox onCheckedChange={(v) => handleCheckboxChange(field.fieldName, item.label, v)} />
                    <h2>{item.label}</h2>
                  </div>
                ))
              ) : (
